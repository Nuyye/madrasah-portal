...
// Tipe data biar TypeScript lu nggak ngamuk
...
export default function RekapAbsensiPage() {
  ...
  // MANTRA PENYEDOT DATA DARI DATABASE
  useEffect(() => {
    const ambilDataSiswa = async () => {
      setIsLoading(true);
      try {
        const response = await fetch(`/api/rekap-absensi?bulan=${selectedBulan}&minggu=${selectedMinggu}`);
  ...
  // MANTRA PENYIMPAN DATA KE DATABASE
  const handleSimpan = async () => {
    ...
    try {
      ...
      if (response.ok) {
        alert(`Mantap Jendral! Data absensi ${selectedBulan} - ${selectedMinggu} berhasil masuk Database!`);
      } else {
        alert("Waduh, servernya nolak jendral!");
      }
    } catch (error) {
      console.error(error);
      alert("Gagal konek ke server nih!");
    } finally {
      setIsSaving(false);
    }
  };
    ...
    <div className="space-y-6 animate-in fade-in duration-500 print:space-y-0 print:m-0 print:p-0">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-100 print:hidden">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <button onClick={handleSimpan} disabled={isSaving} className="flex items-center gap-2 bg-[#115e3b] hover:bg-[#0d4a2e] text-white px-6 py-2.5 rounded-xl font-bold transition-all shadow-sm disabled:opacity-50">
            <Save className="h-5 w-5" /> {isSaving ? "Menyimpan..." : "Simpan Data"}
          </button>
        </div>
      </div>
      ...
      <div className="print-area bg-white rounded-2xl shadow-sm border border-slate-100 p-0 sm:p-2 print:border-none print:shadow-none print:p-0 relative overflow-hidden print:overflow-visible">
        ...
        <div className="hidden print:block w-full">
          <div className="flex items-center justify-between pb-2 relative">
            <div className="w-24 h-24 flex-shrink-0 flex items-center justify-center border border-black rounded-full overflow-hidden">
               <Image src="/assets/logo1.png" alt="Logo Madrasah" width={96} height={96} className="object-contain" />
            </div>
        ...
        <div className="overflow-x-auto print:overflow-visible">
          {isLoading ? (
            <div className="p-10 text-center text-slate-500 font-bold animate-pulse">Menarik Data dari Database...</div>
          ) : (
